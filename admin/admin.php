...
function save_news($list) {
    // сортировка: новые сверху; при равной дате — по id (больше id = создана позже)
...
<?php else: ?>
  ...
  <header>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;">
      <a class="btn btn--ghost" href="?action=new">+ Добавить новость</a>
...
<?php if ($action === 'list'): ?>
  <div class="card">
    <?php if (empty($list)): ?>
      <div class="empty">Пока нет новостей. <a href="?action=new">Добавить первую</a>.</div>
    <?php else: ?>
    <table>
      <thead>
      ...
      <?php foreach ($list as $n): ?>
        <tr>
          ...
          <td class="actions">
            <a class="btn btn--ghost" style="padding:6px 12px;font-size:13px;" href="?action=edit&id=<?php echo (int)$n['id']; ?>">Изменить</a>
